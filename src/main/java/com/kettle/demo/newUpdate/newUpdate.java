 ...
 * @since 26-apr-2003
 */
public class newUpdate extends BaseStep implements StepInterface {
    private static Class<?> PKG = newUpdateMeta.class; // for i18n purposes, needed by Translator2!!

    private newUpdateMeta meta;
    private newUpdateData data;

    public newUpdate(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
                     Trans trans) {
    ...
    public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
        meta = (newUpdateMeta) smi;
        data = (newUpdateData) sdi;
    ...
    public void prepareUpdate(RowMetaInterface rowMeta) throws KettleDatabaseException {
         ...
         * 如果数据更新，etl时间也需要更新
         */
        if (meta.getEtlTime() != null && meta.getEtlTime().length() > 0) {
            if (!Arrays.asList(meta.getUpdateLookup()).contains(meta.getEtlTime())) {   //防止出现set 2次的错误,更新字段中没有才进行更新
                sql += ", " + meta.getEtlTime() + " = now()" + Const.CR;
            }
        }
