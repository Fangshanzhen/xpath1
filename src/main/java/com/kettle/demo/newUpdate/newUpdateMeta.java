...
package com.kettle.demo.newUpdate;
...
@Step(
        ...
        image = "org/pentaho/di/trans/steps/newUpdate/resources/demo.svg",
        categoryDescription = "i18n:org.pentaho.di.trans.step:BaseStep.Category.Transform",
        i18nPackageName = "org.pentaho.di.trans.steps.newUpdate",
        documentationUrl = "DemoStep.DocumentationURL",
        casesUrl = "DemoStep.CasesURL",
        forumUrl = "DemoStep.ForumURL"
)
@InjectionSupported(localizationPrefix = "newUpdateMeta.Injection.", groups = {"KEYS", "UPDATES"})
public class newUpdateMeta extends BaseStepMeta implements StepMetaInterface, ProvidesModelerMeta {
    private static Class<?> PKG = newUpdateMeta.class; // for i18n purposes, needed by Translator2!!
    ...
    public Object clone() {
        newUpdateMeta retval = (newUpdateMeta) super.clone();
    ...
    private void readData(Node stepnode, List<? extends SharedObjectInterface> databases) throws KettleXMLException {
        ...
        try {
            ...
            etlTime = XMLHandler.getTagValue(stepnode, "etl_time");
            index = XMLHandler.getTagValue(stepnode, "index");
            indexName = XMLHandler.getTagValue(stepnode, "index_name");
    ...
    public String getXML() {
        ...
        retval.append("    ").append(XMLHandler.addTagValue("etl_time", etlTime));
        retval.append("    ").append(XMLHandler.addTagValue("index", index));
        retval.append("    ").append(XMLHandler.addTagValue("index_name", indexName));
        retval.append( "    ") .append( XMLHandler.addTagValue( "truncate", truncateTable ) );
    ...
    public void readRep(Repository rep, IMetaStore metaStore, ObjectId id_step, List<DatabaseMeta> databases) throws KettleException {
        ...
        try {
            ...
            etlTime = rep.getStepAttributeString(id_step, "etl_time");
            index = rep.getStepAttributeString(id_step, "index");
            indexName = rep.getStepAttributeString(id_step, "index_name");
    ...
    public void saveRep(Repository rep, IMetaStore metaStore, ObjectId id_transformation, ObjectId id_step) throws KettleException {
        try {
            ...
            rep.saveStepAttribute(id_transformation, id_step, "etl_time", etlTime);
            rep.saveStepAttribute(id_transformation, id_step, "index", index);
            rep.saveStepAttribute(id_transformation, id_step, "index_name", indexName);
...
//                }
                ...
                if (!Utils.isEmpty(tableName)) {
                    ...
                    try {
                        ...
                        String cr_table = db.getDDL(schemaTable, tableFields, null, false, null, true); //----建表----

                        if (cr_table.contains("CREATE")) { //新建表时建索引，修改表时索引不动
                            if (etlTime.length() > 0) {
                                int a = cr_table.lastIndexOf(")"); //最后一个)
                                if (a > 0) {
                                    cr_table = cr_table.replace(cr_table.substring(a), "");
                                }
                                cr_table = cr_table + ",";
                                cr_table = cr_table + etlTime + "  " + "TIMESTAMP " + " NOT NULL DEFAULT CURRENT_TIMESTAMP";
                                cr_table = cr_table + Const.CR + ");";

                            }

                            /**
                             * 新建索引语句，对于text、longtext类型的字段建索引需要指定长度
                             */
                            if (indexName.length() > 0 && index.length() > 0) {
                                for (int i = 0; i < tableFields.size(); i++) {
                                    ValueMetaInterface v = tableFields.getValueMeta(i);
                                    String x = databaseMeta.getFieldDefinition(v, null, null, false); // ipid LONGTEXT
                                    ...
                                    if (index.contains(",")) {   //ipid,pid 复合索引
                                        ...
                                        for (int j = 0; j < indexes.length; j++) {
                                            ...
                                            if (x1 != null && x1.length() > 0) {
                                                if (cr_table.contains(x)) {
                                                    cr_table = cr_table.replace(x, x1);
                                                }
                                            }
                                        }
                                    } else {   //只有一个索引字段
                                        ...
                                        if (x1 != null && x1.length() > 0) {
                                            if (cr_table.contains(x)) {
                                                cr_table = cr_table.replace(x, x1);   //将text类型修改为varchar(255)
                                            }
                                        }
                                    }
                                }
                                if (schemaName.length() > 0) {   //分情况添加索引语句
                                    String indexSql = "CREATE UNIQUE INDEX " + indexName + " ON " + schemaName + "." + tableName + " (" + index + "); ";
                                    cr_table = cr_table + Const.CR + indexSql;
                                } else {
                                    String indexSql = "CREATE UNIQUE INDEX " + indexName + " ON " + tableName + " (" + index + "); ";
                                    cr_table = cr_table + Const.CR + indexSql;
                                }
                            }
...
//            }
    ...
    private String transform(String x, String x1, String in) {
    ...
    public StepInterface getStep(StepMeta stepMeta, StepDataInterface stepDataInterface, int cnr,
        ...
        return new newUpdate(stepMeta, stepDataInterface, cnr, transMeta, trans);
    }

    public StepDataInterface getStepData() {
        return new newUpdateData();
    }
    ...
    public RowMeta getRowMeta(StepDataInterface stepData) {
        return (RowMeta) ((newUpdateData) stepData).insertRowMeta;
    }
